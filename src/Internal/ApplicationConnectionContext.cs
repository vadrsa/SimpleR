...
internal class ApplicationConnectionContext<TMessage> : IWebsocketConnectionContext<TMessage>
{
    ...
    public ApplicationConnectionContext(ConnectionContext context, IMessageWriter<TMessage> writer)
    {
        _connectionContext = context;
        _writer = writer;
        ConnectionAborted = _connectionAbortedTokenSource.Token;
        
        _closedRegistration = _connectionContext.ConnectionClosed.Register(static (state) => ((ApplicationConnectionContext<TMessage>)state!).Abort(), this);
    }
    ...
    private ValueTask<FlushResult> WriteCore(TMessage message, CancellationToken cancellationToken)
    {
        try
        {
    ...
    private async Task WriteSlowAsync(TMessage message, CancellationToken cancellationToken)
    {
    ...
    {
        var connection = (ApplicationConnectionContext<TMessage>)state!;
        ...
        static async Task InnerAbortConnection(ApplicationConnectionContext<TMessage> connection)
        {
